...
pub const Option = struct {
    ...
    pub fn getDefault(comptime self: Option) *const ValueType(self.type) {
        return @as(*const ValueType(self.type), @ptrCast(@alignCast(self.default)));
    }

    pub fn getSettings(comptime self: Option) *const OptionSettings(self.type) {
        return @as(*const OptionSettings(self.type), @ptrCast(@alignCast(self.settings)));
    }
};
...
pub fn option(
    ...
    return .{
        ...
        .long = long,
        .type = T,
        .default = if (T == Flag) &false else @as(*const anyopaque, @ptrCast(&default)),
        .settings = &settings,
    };
}
...
fn structField(
    ...
    return .{
        .name = name,
        .type = T,
        .default_value = @as(?*const anyopaque, @ptrCast(default)),
        .is_comptime = false,
        .alignment = @alignOf(T),
    };
}
